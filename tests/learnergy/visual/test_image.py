...
def test_rasterize():
    ...
    rasterized_array = image._rasterize(
        array.T, img_shape=(d, d), tile_shape=(s, s), tile_spacing=(1, 1)
    )

    assert rasterized_array.shape == (71, 71)

    rasterized_array = image._rasterize(
        array.T, img_shape=(d, d), tile_shape=(s, s), tile_spacing=(1, 1), scale=False
    )
    ...
    rasterized_tuple = image._rasterize(
        tuple, img_shape=(d, d), tile_shape=(s, s), tile_spacing=(1, 1)
    )

    assert rasterized_tuple.shape == (71, 71, 4)

    rasterized_tuple = image._rasterize(
        tuple, img_shape=(d, d), tile_shape=(s, s), tile_spacing=(1, 1), output=False
    )
